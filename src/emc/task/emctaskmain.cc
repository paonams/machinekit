...
static int emcTaskPlan(void)
{
    ...
    if (emcCommand->serial_number != emcStatus->echo_serial_number) {
        ...
        //rcs_print("emcTaskPlan NO NEW Command emcCommand->serial_number (%d)\n",
        //        emcCommand->serial_number);
        type = 0;
    }
    if(type != 0){
	rcs_print("emcTaskPlan task_state :%s  task.mode :%s emcCommand->type (%s)\n",
		emc_task_state_symbol_lookup(emcStatus->task.state),
		emc_task_mode_symbol_lookup(emcStatus->task.mode),
...
static int emcTaskIssueCommand(NMLmsg * cmd)
{
    ...
    if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
        rcs_print("emcTaskIssueCommand Issuing %s -- \t (%s)\n", emcSymbolLookup(cmd->type),
...
static int emcTaskExecute(void)
{
    ...
    //rcs_print("emcTaskExecute task.execState (%s)\n",
    //        emc_task_exec_symbol_lookup(emcStatus->task.execState));
...
static int emctask_startup()
{
    double end;
    int good;
    rcs_print("emctask_startup BEGIN\n");
...
int main(int argc, char *argv[])
{
    ...
    while (!done) {
        ...
        if (0 != emcCommandBuffer->peek()) {
            ...
            taskPlanError = 0;
            taskExecuteError = 0;
            //rcs_print("MAIN NEW COMMAND ---------------------------------- \n");
        }
        else{
            //rcs_print("still the old command \n");
        }
        // run control cycle
        if (0 != emcTaskPlan()) {
            taskPlanError = 1;
