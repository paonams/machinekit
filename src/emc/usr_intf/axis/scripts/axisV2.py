...
class MyOpengl(GlCanonDraw, Opengl):
    ...
    def set_current_line(self, line):
        ...
        widgets_text.tag_remove("executing", "0.0", "end")
        if line is not None and line > 0:
            vupdate(vars.running_line, line)
            if vars.highlight_line.get() <= 0:
                widgets_text.see("%d.0" % (line+2))
                widgets_text.see("%d.0" % line)
            widgets_text.tag_add("executing", "%d.0" % line, "%d.end" % line)
        else:
    ...
    def set_highlight_line(self, line):
        ...
        widgets_text.tag_remove("sel", "0.0", "end")
        if line is not None and line > 0:
            widgets_text.see("%d.0" % (line+2))
            widgets_text.see("%d.0" % line)
            widgets_text.tag_add("sel", "%d.0" % line, "%d.end" % line)
...
def toggle_perspective(e):
    gMyOpengl.perspective = not gMyOpengl.perspective
    gMyOpengl.tkRedraw()

def select_line(event):
    i = widgets_text.index("@%d,%d" % (event.x, event.y))
    i = int(i.split('.')[0])
    gMyOpengl.set_highlight_line(i)
    gMyOpengl.tkRedraw()
    return "break"

def select_prev(event):
    if gMyOpengl.highlight_line is None:
        i = gMyOpengl.last_line
    else:
        i = max(1, gMyOpengl.highlight_line - 1)
    gMyOpengl.set_highlight_line(i)
    gMyOpengl.tkRedraw()
...
def select_next(event):
    if gMyOpengl.highlight_line is None:
        ...
        i = min(o.last_line, gMyOpengl.highlight_line + 1)
    gMyOpengl.set_highlight_line(i)
    gMyOpengl.tkRedraw()

def scroll_up(event):
    widgets_text.yview_scroll(-2, "units")
...
class LivePlotter:
    ...
    def start(self):
        ...
        def C(s):
            a = gMyOpengl.colors[s + "_alpha"]
            s = gMyOpengl.colors[s]
        ...
        gMyOpengl.after_idle(lambda: thread.start_new_thread(self.logger.start, (.01,)))
    ...
    def update(self):
        ...
        if (self.logger.npts != self.lastpts
                or limits != gMyOpengl.last_limits
                or self.stat.actual_position != gMyOpengl.last_position
                or self.stat.joint_actual_position != gMyOpengl.last_joint_position
                or self.stat.homed != gMyOpengl.last_homed
                or self.stat.g5x_offset != gMyOpengl.last_g5x_offset
                or self.stat.g92_offset != gMyOpengl.last_g92_offset
                or self.stat.g5x_index != gMyOpengl.last_g5x_index
                or self.stat.rotation_xy != gMyOpengl.last_rotation_xy
                or self.stat.limit != gMyOpengl.last_limit
                or self.stat.tool_table[0] != gMyOpengl.last_tool
                or self.stat.motion_mode != gMyOpengl.last_motion_mode
                or abs(speed - self.last_speed) > .01):
            gMyOpengl.redraw_soon()
            gMyOpengl.last_limits = limits
            gMyOpengl.last_limit = self.stat.limit
            gMyOpengl.last_homed = self.stat.homed
            gMyOpengl.last_position = self.stat.actual_position
            gMyOpengl.last_g5x_offset = self.stat.g5x_offset
            gMyOpengl.last_g92_offset = self.stat.g92_offset
            gMyOpengl.last_g5x_index = self.stat.g5x_index
            gMyOpengl.last_rotation_xy = self.stat.rotation_xy
            gMyOpengl.last_motion_mode = self.stat.motion_mode
            gMyOpengl.last_tool = self.stat.tool_table[0]
            gMyOpengl.last_joint_position = self.stat.joint_actual_position
...
def running(do_poll=True):
    if do_poll: linuxcnc_stat.poll()
    return linuxcnc_stat.task_mode == linuxcnc.MODE_AUTO and linuxcnc_stat.interp_state != linuxcnc.INTERP_IDLE
...
This means this function returns True when the mdi tab is visible."""
    if do_poll: linuxcnc_stat.poll()
    if linuxcnc_stat.task_state != linuxcnc.STATE_ON: return False
    return linuxcnc_stat.interp_state == linuxcnc.INTERP_IDLE or (linuxcnc_stat.task_mode == linuxcnc.MODE_MDI and linuxcnc_stat.queued_mdi_commands < vars.max_queued_mdi_commands.get())

# If LinuxCNC is not already in one of the modes given, switch it to the
# first mode
def ensure_mode(m, *p):
    linuxcnc_stat.poll()
    if linuxcnc_stat.task_mode == m or linuxcnc_stat.task_mode in p: return True
...
class Progress:
    def __init__(self, phases, total):
        ...
        widgets_text.configure(cursor="watch")
    ...
    def update(self, count, force=0):
        if force or count - self.lastcount > 400:
            ...
            widgets_text.tk.call(".info.progress", "coords", "1",
                (0, 0, int(fraction * width), height))
            widgets_text.tk.call("update", "idletasks")
    ...
    def done(self):
        ...
        widgets_text.configure(cursor="xterm")
...
class AxisCanon(GLCanon, StatMixin):
    def __init__(self, widget, text, linecount, progress, arcdivision):
        ...
        StatMixin.__init__(self, linuxcnc_stat, random_toolchanger)
        self.text = text
...
def cancel_open(event=None):
    if gMyOpengl.canon is not None:
        gMyOpengl.canon.aborted = True
...
def open_file_guts(f, filtered=False, addrecent=True):
    ...
    canon = None
    gMyOpengl.deselect(None) # remove highlight line from last program
    try:
        ...
        cnc_cmd.wait_complete()
        cnc_cmd.program_open(f)
        lines = open(f).readlines()
        progress = Progress(2, len(lines))
        widgets_text.configure(state="normal")
        widgets_text.tk.call("delete_all", widgets_text)
        code = []
        i = 0
        ...
        f = os.path.abspath(f)
        gMyOpengl.canon = canon = AxisCanon(gMyOpengl, widgets.text, i, progress, arcdivision)
        ...
        if allow_preview :
            try:
                result, seq = gMyOpengl.load_preview(f, canon, unitcode, initcode, interpname)
        ...
        widgets_text.configure(state="disabled")
        gMyOpengl.lp.set_depth(from_internal_linear_unit(o.get_foam_z()),
...
def set_first_line(lineno):
    ...
    widgets_text.tag_remove("ignored", "0.0", "end")
    if lineno > 0:
        widgets_text.tag_add("ignored", "0.0", "%d.end" % (lineno-1))
...
class _prompt_touchoff(_prompt_float):
    def __init__(self, title, text_pattern, default, defaultsystem):
        systems = all_systems[:]
        if linuxcnc_stat.tool_in_spindle == 0:
            del systems[-1]
...
def run_warn():
    ...
    if gMyOpengl.canon:
        ...
        for i in range(3): # Does not enforce angle limits
            if not(linuxcnc_stat.axis_mask & (1<<i)): continue
            if gMyOpengl.canon.min_extents_notool[i] < machine_limit_min[i]:
                warnings.append(_("Program exceeds machine minimum on axis %s")
                    % "XYZABCUVW"[i])
            if gMyOpengl.canon.max_extents_notool[i] > machine_limit_max[i]:
...
def reload_file(refilter=True):
    ...
    gMyOpengl.set_highlight_line(None)
    ...
    if line:
        gMyOpengl.set_highlight_line(line)
...
#---------------------------------------------------------
    ...
    def toggle_optional_stop(event=None):
        cnc_cmd.set_optional_stop(vars.optional_stop.get())
        ap.putpref("optional_stop", vars.optional_stop.get())

    def toggle_block_delete(event=None):
        cnc_cmd.set_block_delete(vars.block_delete.get())
    ...
    def gcode_properties(event=None):
        ...
        if not loaded_file:
            ...
            #print gMyOpengl.canon.traverse[0]

            g0 = sum(dist(l[1][:3], l[2][:3]) for l in gMyOpengl.canon.traverse)
            g1 = (sum(dist(l[1][:3], l[2][:3]) for l in gMyOpengl.canon.feed) +
                sum(dist(l[1][:3], l[2][:3]) for l in gMyOpengl.canon.arcfeed))
            gt = (sum(dist(l[1][:3], l[2][:3])/min(mf, l[3]) for l in gMyOpengl.canon.feed) +
                sum(dist(l[1][:3], l[2][:3])/min(mf, l[3])  for l in gMyOpengl.canon.arcfeed) +
                sum(dist(l[1][:3], l[2][:3])/mf  for l in gMyOpengl.canon.traverse) +
                gMyOpengl.canon.dwell_time
                )
            ...
            min_extents = from_internal_units(gMyOpengl.canon.min_extents, conv)
            max_extents = from_internal_units(gMyOpengl.canon.max_extents, conv)
    ...
    def set_view_p(event=None):
        ...
        gMyOpengl.set_view_p()

    def estop_clicked(event=None):
        linuxcnc_stat.poll()
        if linuxcnc_stat.task_state == linuxcnc.STATE_ESTOP:
    ...
    def onoff_clicked(event=None):
        linuxcnc_stat.poll()
        if linuxcnc_stat.task_state == linuxcnc.STATE_ESTOP_RESET:
    ...
    def remote (cmd,arg=""):
        ...
        if cmd == "open_file_name":
            ...
            ensure_mode(linuxcnc.MODE_AUTO)
            cnc_cmd.auto(linuxcnc.AUTO_RUN, program_start_line)
    ...
    def open_file_name(f):
        ...
        if str(widgets.view_x['relief']) == "sunken":
            ...
            commands.set_view_p()
        if gMyOpengl.canon is not None:
            x = (gMyOpengl.canon.min_extents[0] + gMyOpengl.canon.max_extents[0])/2
            y = (gMyOpengl.canon.min_extents[1] + gMyOpengl.canon.max_extents[1])/2
            z = (gMyOpengl.canon.min_extents[2] + gMyOpengl.canon.max_extents[2])/2
            gMyOpengl.set_centerpoint(x, y, z)
    ...
    def task_step(*event):
        if linuxcnc_stat.task_mode != linuxcnc.MODE_AUTO or linuxcnc_stat.interp_state != linuxcnc.INTERP_IDLE:
            gMyOpengl.set_highlight_line(None)
            if run_warn(): return
        ensure_mode(linuxcnc.MODE_AUTO)
    ...
    def task_pause(*event):
        if linuxcnc_stat.task_mode != linuxcnc.MODE_AUTO or linuxcnc_stat.interp_state not in (linuxcnc.INTERP_READING, linuxcnc.INTERP_WAITING):
            return
        ensure_mode(linuxcnc.MODE_AUTO)
    ...
    def task_resume(*event):
        linuxcnc_stat.poll()
        if not linuxcnc_stat.paused:
            return
        if linuxcnc_stat.task_mode not in (linuxcnc.MODE_AUTO, linuxcnc.MODE_MDI):
            return
    ...
    def task_pauseresume(*event):
        ...
        ensure_mode(linuxcnc.MODE_AUTO, linuxcnc.MODE_MDI)
        linuxcnc_stat.poll()
        if linuxcnc_stat.paused:
            cnc_cmd.auto(linuxcnc.AUTO_RESUME)
        elif linuxcnc_stat.interp_state != linuxcnc.INTERP_IDLE:
            cnc_cmd.auto(linuxcnc.AUTO_PAUSE)

    def task_stop(*event):
        if linuxcnc_stat.task_mode == linuxcnc.MODE_AUTO and vars.running_line.get() != 0:
            gMyOpengl.set_highlight_line(vars.running_line.get())
        cnc_cmd.abort()
        cnc_cmd.wait_complete()
    ...
    def home_all_axes(event=None):
        ...
        for i,h in enumerate(s.homed):
            if linuxcnc_stat.axis_mask & (1<<i):
    ...
    def home_axis(event=None):
        if not manual_ok(): return
        doHoming=True
        if linuxcnc_stat.homed["xyzabcuvw".index(vars.current_axis.get())]:
            doHoming=prompt_areyousure(_("Warning"),_("This axis is already homed, are you sure you want to re-home?"))
        if doHoming:
            ensure_mode(linuxcnc.MODE_MANUAL)
            cnc_cmd.home("xyzabcuvw".index(vars.current_axis.get()))

    def unhome_axis(event=None):
        if not manual_ok(): return
        ensure_mode(linuxcnc.MODE_MANUAL)
        cnc_cmd.unhome("xyzabcuvw".index(vars.current_axis.get()))
    ...
    def clear_offset(num):
        ...
        if num == "G92":
            ...
            for i, a in enumerate("XYZABCUVW"):
                if linuxcnc_stat.axis_mask & (1<<i): clear_command += " %c0" % a
        cnc_cmd.mdi(clear_command)
        cnc_cmd.wait_complete()
        ensure_mode(linuxcnc.MODE_MANUAL)
        linuxcnc_stat.poll()
        gMyOpengl.tkRedraw()
    ...
    def touch_off(event=None, new_axis_value = None):
        ...
        if system.split()[0] == "T":
            ...
            offset_command = "G10 L%d P%d %c[%s*%.12f]" % (lnum, linuxcnc_stat.tool_in_spindle, vars.current_axis.get(), new_axis_value, scale)
            cnc_cmd.mdi(offset_command)
            cnc_cmd.wait_complete()
            cnc_cmd.mdi("G43")
            cnc_cmd.wait_complete()
        else:
    ...
    def brake(event=None):
        if not manual_ok(): return
        ensure_mode(linuxcnc.MODE_MANUAL)
        cnc_cmd.brake(vars.brake.get())
    def flood(event=None):
        cnc_cmd.flood(vars.flood.get())
    def mist(event=None):
        cnc_cmd.mist(vars.mist.get())
    def spindle(event=None):
        if not manual_ok(): return
        ensure_mode(linuxcnc.MODE_MANUAL)
        cnc_cmd.spindle(vars.spindledir.get())
    def spindle_increase(event=None):
        cnc_cmd.spindle(linuxcnc.SPINDLE_INCREASE)
    def spindle_decrease(event=None):
        cnc_cmd.spindle(linuxcnc.SPINDLE_DECREASE)
    def spindle_constant(event=None):
        if not manual_ok(): return
        ensure_mode(linuxcnc.MODE_MANUAL)
        cnc_cmd.spindle(linuxcnc.SPINDLE_CONSTANT)
    def set_first_line(lineno):
        if not manual_ok(): return
    ...
    def mist_toggle(*args):
        linuxcnc_stat.poll()
        cnc_cmd.mist(not linuxcnc_stat.mist)
    def flood_toggle(*args):
        linuxcnc_stat.poll()
        cnc_cmd.flood(not linuxcnc_stat.flood)

    def spindle_forward_toggle(*args):
        if not manual_ok(): return
        linuxcnc_stat.poll()
        if linuxcnc_stat.spindle_direction == 0:
            cnc_cmd.spindle(1)
        else:
            cnc_cmd.spindle(0)
    ...
    def toggle_override_limits(*args):
        linuxcnc_stat.poll()
        if linuxcnc_stat.interp_state != linuxcnc.INTERP_IDLE: return
        if linuxcnc_stat.axis[0]['override_limits']:
            ensure_mode(linuxcnc.MODE_AUTO)
        else:
            ensure_mode(linuxcnc.MODE_MANUAL)
            cnc_cmd.override_limits()
    ...
    def set_joint_mode(*args):
        ...
        cnc_cmd.teleop_enable(joint_mode)
        cnc_cmd.wait_complete()
    ...
    def goto_sensible_line():
        line = gMyOpengl.get_highlight_line()
...
def activate_axis_or_set_feedrate(n):
    ...
    if manual_ok() and linuxcnc_stat.axis_mask & (1<<n):
        activate_axis(n)
    else:
        set_feedrate(10*n)
...
def jog_on(a, b):
    ...
    if jog_after[a]:
        ...
        return
    jogincr = widgets.jogincr.get()
    if linuxcnc_stat.motion_mode == linuxcnc.TRAJ_MODE_TELEOP:
        jogging[a] = b
        jog_cont[a] = False
        cartesian_only=jogging[:6]
        cnc_cmd.teleop_vector(*cartesian_only)
    else:
        if jogincr != _("Continuous"):
            linuxcnc_stat.poll()
            if s.state != 1: return
...
statfail=0
statwait=.01
while linuxcnc_stat.axes == 0:
    print "waiting for linuxcnc_stat.axes"
    time.sleep(statwait)
    statfail+=1
    statwait *= 2
    if statfail > 8:
...
for i,j in enumerate("XYZABCUVW"):
    if linuxcnc_stat.axis_mask & (1<<i) == 0: continue
    live_axis_count += 1
...
for i in range(9):
    if linuxcnc_stat.axis_mask & (1<<i): continue
...
if linuxcnc_stat.axis_mask & 56 == 0:
    widgets.ajogspeed.grid_forget()
#c = linuxcnc.command()
#e = linuxcnc.error_channel()
cnc_cmd = linuxcnc.command()
cnc_error = linuxcnc.error_channel()
...
gMyOpengl = MyOpengl(widgets.preview_frame, width=400, height=300, double=1, depth=1)
gMyOpengl.last_line = 1
gMyOpengl.pack(fill="both", expand=1)
...
#t = widgets.text
widgets_text = widgets.text
widgets_text.bind('<Button-3>', rClicker) #allow right-click to select start from line
widgets_text.tag_configure("ignored", background="#ffffff", foreground="#808080")
widgets_text.tag_configure("lineno", foreground="#808080")
widgets_text.tag_configure("executing", background="#804040", foreground="#ffffff")
widgets_text.bind("<Button-1>", select_line)
widgets_text.bind("<B1-Motion>", lambda e: "break")
widgets_text.bind("<B1-Leave>", lambda e: "break")
widgets_text.bind("<Button-4>", scroll_up)
widgets_text.bind("<Button-5>", scroll_down)
widgets_text.configure(state="disabled")
...
live_plotter = LivePlotter(gMyOpengl)
live_plotter.start()
gMyOpengl.lp = live_plotter.logger
hershey = Hershey()
...
if linuxcnc_stat.kinematics_type != linuxcnc.KINEMATICS_IDENTITY:
