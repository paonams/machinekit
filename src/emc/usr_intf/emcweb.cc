...
void GenerateJson(const char *filename)
{
    ...
    if (pCtrl->GetInitState() == miniemc::EmcController::lsLoaded) {
        ...
        //std::cout<<"GenerateJson buff "<<buff<<"\n";
    }
}
...
int main(int argc, char* argv[])
{
    ...
    rcs_print("emcweb emc_initfile (%s)\n", emc_inifile);
    ...
    if (NULL != (inistring = inifile.Find("ROOT", "EMCWEB"))) {
        wwwroot = inistring;
        std::cout<<"ROOT EMCWEB wwwroot "<<wwwroot<<"\n";
    }

    if (NULL != (inistring = inifile.Find("PORT", "EMCWEB"))) {
        wwwport = inistring;
        std::cout<<"PORT EMCWEB wwwport "<<wwwport<<"\n";
    }

    if (NULL != (inistring = inifile.Find("NC_FILES_DIR", "EMCWEB"))) {
        ncFilesDir = inistring;
        std::cout<<"NC_FILES_DIR EMCWEB ncFilesDir "<<ncFilesDir<<"\n";
    }
    ...
    rcs_print("emcweb emcCoord (%s) emcjson (%s)\n", emcCoord.c_str(), emcCFGJson.c_str());
    try {
        ...
        while (!bFinish) {
            if (pCtrl->Update()) {
                //rcs_print("emcweb got new update\n");
